...
.update({ user_name: user_name, role_id: role, email: email, password: hashedPassword,}) 
.then((results) => {
res.redirect("/users");
});
});
...
router.post('/users', upload.single('photo'), async (req, res) => {
  ...
  let email = req.body.email;
  knex('admin.users', )
          .insert({user_name: name, password: hashedPassword, photo: photo, role_id: role, email: email})
          .then((results) => {
                  res.redirect('/users');
          });
});
